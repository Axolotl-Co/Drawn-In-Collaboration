...
const parseExtDataUri = (uri) => {
    ...
    const buf = (0, base64_1.fromBase64)(uri.substring(commaIndex + 1));
            ...
            switch (typeof item) {
                ...
                case 'string': {
                    if (item.length < minDataUri)
                        continue;
                    if (item.substring(0, binUriStartLength) === constants_1.binUriStart) {
                        const buf = (0, base64_1.fromBase64)(item.substring(binUriStartLength));
                        value[key] = buf;
                    }
                    else if (item.substring(0, msgPackUriStartLength) === constants_1.msgPackUriStart) {
...
exports.parse = parse;
const stringifyBinary = (value) => (constants_1.binUriStart + (0, base64_1.toBase64)(value));
